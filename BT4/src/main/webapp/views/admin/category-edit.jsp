            ...
            <div class="mb-3">
                ...
                <c:choose>
                    ...
                    <c:when test="${fn:startsWith(category.images, 'https://') || fn:startsWith(category.images, 'http://')}">
                        <img class="icon" src="<c:out value='${category.images}'/>" alt="icon"/>
                    </c:when>
                    <c:otherwise>
                        <c:url var="categoryImageUrl" value="/image">
                            <c:param name="fname" value="${category.images}"/>
                        </c:url>
                        <img class="icon" src="${categoryImageUrl}" alt="icon"/>
                    </c:otherwise>
                </c:choose>
            </div>

            <div class="mb-3">
                <label class="form-label" for="images1">Chọn ảnh mới</label>
